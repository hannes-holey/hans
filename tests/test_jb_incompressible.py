...
class TestIncompressibleJournalBearing(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        config_file = os.path.join("examples", "jb_inf_stiffDH.yaml")
        ...
        ds = netCDF4.Dataset(os.path.join(cls.tmp_dir, "jb_inf_stiffDH_0001.nc"))
        cls.rho = ds.variables["rho"]
        cls.mass = ds.variables["mass"]

        cls.rho_ref, cls.p_ref = np.loadtxt(os.path.join("tests", "jb_0.7_inf_stiffDH_ref.dat"), unpack=True)
