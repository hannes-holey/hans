...
class Solver:
    ...
    def __init__(self, options, disc, geometry, numerics, material):
        ...
        # self.fluxX_E = VectorField(disc)
        # self.fluxX_W = VectorField(disc)
        # self.fluxY_N = VectorField(disc)
        # self.fluxY_S = VectorField(disc)
...
#        self.alp.field[0] =  6.*self.mu/(self.q.field[2] * self.height.field[0]**2) * (self.U*self.q.field[2] - 2.* self.q.field[0])
    ...
    def plot(self, field):
        ...
        if self.plot_dim == 1:
            ...
            self.line3, = self.ax1[1,1].plot(x, DowsonHigginson(self.material).isoT_pressure(field.field[2][:,int(self.Ny/2)]))
    ...
    def animate1D(self, i, field):
        ...
        if i%self.plotInterval == 0:
            ...
            limits[3,0] = np.amin(DowsonHigginson(self.material).isoT_pressure(field.field[2][:,int(self.Ny/2)]))
            limits[3,1] = np.amax(DowsonHigginson(self.material).isoT_pressure(field.field[2][:,int(self.Ny/2)]))
            ...
            self.line3.set_ydata(DowsonHigginson(self.material).isoT_pressure(field.field[2][:,int(self.Ny/2)]))
    ...
    def animate2D(self, i, field):
        if i%self.plotInterval == 0:
            ...
            self.im3.set_array(DowsonHigginson(self.material).isoT_pressure(field.field[2].T))
