...
Usage:
    python GaPFlow/solver_fem/tests/interactive_residual.py
...
print(f'P2 inner: {Nx_P2}x{Ny_P2}, P2 padded: {Nx_v_pad}x{Ny_v_pad}')
print(f'jx DOF range: [{jx_slice.start}, {jx_slice.stop})')
print(f'Inner node (ix,iy) -> flat index: ix + iy*{Nx_P2}')
print()
...
while True:
    ...
    if cmd == 'r':
        ...
        try:
            ...
            if ix < 0 or ix >= Nx_P2 or iy < 0 or iy >= Ny_P2:
                print(f'Out of range: ix in [0,{Nx_P2-1}], iy in [0,{Ny_P2-1}]')
                continue
            flat_idx = ix + iy * Nx_P2
    ...
    print_field_2d('jx (inner)', jx_inner, Nx_P2, Ny_P2)
