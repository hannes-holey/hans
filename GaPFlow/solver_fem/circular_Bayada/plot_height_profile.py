...
def latest_run_dir(data_dir):
    runs = sorted(glob.glob(os.path.join(data_dir, '*_circular_Bayada')))
    if not runs:
...
def load_grid(run_dir):
    ...
    with open(config_path) as f:
        # config.yml may contain numpy-specific YAML tags — load as raw text
        raw = f.read()
    # Fall back to parsing the original YAML for grid info
    yaml_path = os.path.join(os.path.dirname(__file__), 'circular_Bayada.yaml')
...
with nc.Dataset(topo_path) as f:
    # shape: (frame, component, 1, nx, ny)
    # component 0 = h
    topo = f.variables['topography'][:]

h_last = topo[-1, 0, 0, :, :]  # last frame, h component, (nx, ny)
d_last = topo[-1, 3, 0, :, :]  # last frame, deformation component, (nx, ny)
