...
Usage:
    /home/qd5728/fem_taylor_hood/venv/bin/python \
        GaPFlow/fem_2d/tests/inspect_vp_connectivity.py
"""
...
if __name__ == '__main__':
    ...
    Nx_v, Ny_v = gi.Nx_v_inner, gi.Ny_v_inner

    print(f'Grid: {Nx}x{Ny}, bc={bc}')
    print(f'P1 inner: {Nx_p}x{Ny_p},  P2 inner: {Nx_v}x{Ny_v}')
    ...
    print(f'\nP2 inner mask:')
    print(gi.index_mask_inner_local_v)
    ...
    FROM_v = gi.sq_FROM_padded_v('jx')
    vp_pairs = collect_pairs(gi, P1, P2, TO_p, FROM_v)
    print_connectivity(vp_pairs, 'mass', 'jx', Nx_p, Nx_v)
    ...
    TO_v = gi.sq_TO_inner_v
    FROM_p = gi.sq_FROM_padded_p('rho')
    pv_pairs = collect_pairs(gi, P2, P1, TO_v, FROM_p)
    print_connectivity(pv_pairs, 'mom_x', 'rho', Nx_v, Nx_p)
