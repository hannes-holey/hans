...
class StubDecomp:
    def __init__(self, Nx_p, Ny_p):
        ...
        self.index_mask_padded_global_v = np.full((Nx_v+4, Ny_v+4), -1, dtype=np.int32)
        self.index_mask_padded_global_v[2:-2, 2:-2] = np.arange(Nx_v*Ny_v).reshape(Nx_v, Ny_v, order='F')
...
def apply_stencil(stencil_block, block_type, m_inner_v, m_padded_v, m_padded_p):
    res_grid, var_grid = block_type
    inner_pts_2d = np.argwhere(m_inner_v >= 0)
...
if __name__ == '__main__':
    ...
    nb_v = (2*Nx_p-1) * (2*Ny_p-1)
    nb_p = Nx_p * Ny_p

    print('block        nnz   inner_range  contrib_range  no_dups')
    all_ok = True
    for bi, bt in enumerate(BLOCK):
        inner, contrib = apply_stencil(stencil[bi], bt, m_inner_v, m_padded_v, m_padded_p)
        nb_i = nb_v if bt[0] == 'v' else nb_p
        nb_c = nb_v if bt[1] == 'v' else nb_p
