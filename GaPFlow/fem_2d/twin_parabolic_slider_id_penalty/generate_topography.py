"""
Generate topography for the twin identical parabolic slider (penalty variant).

Reference: Giacopini et al. (2010), "A mass-conserving complementarity
formulation to study lubricant films in the presence of cavitation",
Fig. 1a / Fig. 2 (double parabolic slider case).
...
Geometry parameters:
  hmax = 50.2 um
  hmin = 25.4 um  (both sliders identical)
"""
...
out_file = os.path.join(out_dir, 'twin_parabolic_slider_id_penalty.npy')
...
ax.set_xlabel('x [mm]')
ax.set_ylabel('h [um]')
ax.set_title('Twin identical parabolic slider (penalty) — height profile')
ax.set_xlim(x_plot[0], x_plot[-1])
ax.set_ylim(0, h_top * 1.25)
ax.grid(True, linestyle=':', alpha=0.4)

fig.tight_layout()
plot_file = os.path.join(out_dir, 'twin_parabolic_slider_id_penalty.png')
