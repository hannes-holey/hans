...
# ── Bayada EOS: compute Pcav and rho_BC for p_amb = 1e5 Pa ────────────────
rho_l = 850.0
rho_v = 0.019
c_l   = 5989.28
c_v   = 352.0
...
# Liquid branch: p = Pcav + (rho - rho_l) * c_l^2  =>  rho = rho_l + (p - Pcav) / c_l^2
rho_BC = rho_l + (p_amb - Pcav) / c_l**2

print(f"Bayada EOS:  Pcav = {Pcav:.1f} Pa")
print(f"rho at p_amb = {p_amb:.0f} Pa:  rho_BC = {rho_BC:.6f} kg/m³")

# ── Save ──────────────────────────────────────────────────────────────────
out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'topography')
os.makedirs(out_dir, exist_ok=True)
out_file = os.path.join(out_dir, 'conv_slider_pocket_2D.npy')
...
fig.tight_layout()
plot_file = os.path.join(out_dir, 'conv_slider_pocket_2D.png')
