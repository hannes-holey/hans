"""
Generate topography for the twin identical parabolic slider.
...
Geometry parameters:
  hmax = 50.2 µm
  hmin = 25.4 µm  (both sliders identical)
"""
...
out_file = os.path.join(out_dir, 'twin_parabolic_slider_id.npy')
...
ax.set_xlabel('x [mm]')
ax.set_ylabel('h [µm]')
ax.set_title('Twin identical parabolic slider — height profile')
ax.set_xlim(x_plot[0], x_plot[-1])
ax.set_ylim(0, h_top * 1.25)
ax.grid(True, linestyle=':', alpha=0.4)

fig.tight_layout()
plot_file = os.path.join(out_dir, 'twin_parabolic_slider_id.png')
