...
out_file = os.path.join(out_dir, 'twin_parabolic_slider_penalty.npy')
...
ax.annotate(f'hmin1 = {hmin1*1e6:.0f} um', xy=(x_s1_start * 1e3 + 18, hmin1 * 1e6),
            xytext=(x_s1_start * 1e3 + 18, hmin1 * 1e6 - 6),
            fontsize=9, color='steelblue', ha='center')
ax.annotate(f'hmin2 = {hmin2*1e6:.0f} um', xy=(x_s2_start * 1e3 + 18, hmin2 * 1e6),
            xytext=(x_s2_start * 1e3 + 18, hmin2 * 1e6 - 6),
            fontsize=9, color='steelblue', ha='center')

ax.set_xlabel('x [mm]')
ax.set_ylabel('h [um]')
ax.set_title('Twin parabolic slider (penalty) — height profile')
...
fig.colorbar(im, ax=ax2, label='h [um]')
ax2.set_xlabel('x [mm]')
ax2.set_ylabel('y [mm]')
ax2.set_title('2-D topography (constant in y)')

fig.tight_layout()
plot_file = os.path.join(out_dir, 'twin_parabolic_slider_penalty.png')
